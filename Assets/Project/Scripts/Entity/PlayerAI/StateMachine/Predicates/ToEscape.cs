...
{
    public class ToEscape : IPredicate
    {
        ...
        public ToEscape(AIStateContext context,AISpawner aiSpawner)
        {
        ...
        private void GetPlayersInRange()
        {
            ...
            for (int i = 0; i < m_aiLookupBuffer.Length; i++)
            {
                ...
                if (distance <= m_stateContext.AISettings.RunAwayVisionThreshold)
                {
            ...
            if (playerDistance <= m_stateContext.AISettings.RunAwayVisionThreshold)
            {
        ...
        {
            if (m_stateContext.Entity.GetPercentHealth() <= m_stateContext.AISettings.IsDangerHealthPercentageThreshold)
            {
                return true;
            }

            if (m_stateContext.AISettings.IsDangerWeaponAdvantageImportant)
            {
                foreach (PlayerEntity player in m_playersInRangeBuffer)
                {
                    ...
                    if (entityWeaponCount < weaponCount)
                    {
                        return true;
                    }
                }

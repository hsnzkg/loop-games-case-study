...
public class EndCardController : MonoBehaviour
{
    ...
    private ScreenOrientationType get_getCurrentScreenOrientationType()
    {
    ...
    void CreateInstance()
    {
        ...
        if (Instance != this)
        {
            Debug.LogWarning("Duplicate EndCardController destroyed");
            Destroy(gameObject);
        }
    }
    ...
    {
        float buttonPositionX = IsPortrait() ? _buttonPositionX : _buttonPositionXLandscape;
        float buttonPositionY = IsPortrait() ? _buttonPositionXLandscape : _buttonPositionYLandscape;
...
#endif
#if !UNITY_EDITOR
    if (_currentScreenType != get_getCurrentScreenOrientationType())
        {
            _currentScreenType = get_getCurrentScreenOrientationType();
            SetCTAButtonPosition();
            SetFontSize();
            SetPositionIcon();
            SetPositionDescriptionText();
    ...
    {
        if (!_ScreenCTAButton) Debug.LogError("_ScreenCTAButton NULL", this);
        if (!_CTAButton) Debug.LogError("_CTAButton NULL", this);
        if (!_CTAButtonAnimator) Debug.LogError("_CTAButtonAnimator NULL", this);
        if (!_endCardAnimator) Debug.LogError("_endCardAnimator NULL", this);
        if (!_backgroundImage) Debug.LogError("_backgroundImage NULL", this);
        if (!_iconImage) Debug.LogError("_iconImage NULL", this);
    }
